...
export function Url(options: UrlOptions = {}): Codec<URL> {
  if (options.allowedProtocols) {
    for (const protocol of options.allowedProtocols) {
      if (!protocol.endsWith(`:`)) {
        throw new Error(`Invalid protocol, "${protocol}". Did you mean: "${protocol}:"?`);
      }
    }
  }
  return Constraint(
...
function getErrorFor(
  ...
  if (set && !set.has(value[name])) {
    return `Invalid ${name} ${showValue(value[name])}, expected one of: ${[...set]
      .map(v => showValue(v))
